...
class IngredientService:
    ...
    async def trigger_analysis(
        ...
        # 提取 ingredient 数据，避免 ORM 对象跨 session 引用
        ingredient_dict = {
        ...
        background_tasks.add_task(
            _run_ingredient_analysis_workflow,
...
# ── 独立的后台任务函数（在 api/ingredients/ 包外，不属于 L1 Service 层）────────────
# 此函数由 BackgroundTasks.add_task() 调用，运行于请求周期之外，
# 因此自行管理 session 生命周期，不违反 T5 规则。
