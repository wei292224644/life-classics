...
from workflow_parser_kb.nodes.classify_node import (
    classify_node,
    _escape_for_json_prompt,
    _replace_latex_with_placeholders,
    _restore_placeholders,
)
from workflow_parser_kb.nodes.output import ClassifyOutput, SegmentItem
...
async def test_classify_node_produces_dual_type_segments(tmp_path):
    ...
    with patch("workflow_parser_kb.nodes.classify_node.invoke_structured", return_value=mock_output):
...
async def test_classify_node_prompt_includes_both_type_lists(tmp_path):
    ...
    with patch("workflow_parser_kb.nodes.classify_node.invoke_structured", side_effect=capture_invoke):
...
async def test_classify_node_html_chunk_prompt_escapes_attribute_quotes(tmp_path):
    ...
    with patch(
        "workflow_parser_kb.nodes.classify_node.invoke_structured",
...
def test_classify_llm_sends_placeholder_not_latex(tmp_path):
    ...
    from workflow_parser_kb.nodes.classify_node import _call_classify_llm
    from workflow_parser_kb.rules import RulesStore
...
async def test_classify_node_concurrent_execution_with_exceptions(tmp_path):
    ...
    from workflow_parser_kb.models import WorkflowState, ClassifiedChunk, TypedSegment
    from workflow_parser_kb.nodes.output import ClassifyOutput, SegmentItem
    ...
    # classify_raw_chunk 返回 ClassifiedChunk（TypedDict），不是 ClassifyOutput
    mock_classified_chunk = ClassifiedChunk(
        raw_chunk={"content": "chunk", "section_path": ["A.1"], "char_count": 6},
    ...
    with patch("workflow_parser_kb.nodes.classify_node.asyncio.to_thread", side_effect=mock_to_thread):
        result = await classify_node(state)
