...
**Goal:** 实现从用户拍照到产品分析结果页的完整分析管道，包含 OCR、成分解析、配料库匹配、LangGraph 产品分析 Agent，以及 IngredientAnalysis 离线生成 workflow。

**Architecture:** 四组件顺序管道（OCR → Parse → Match → Analyze）通过 Redis 任务状态机串联，对外暴露 `POST /api/analysis/start` + `GET /api/analysis/{task_id}/status` + `POST /api/analysis/feedback` 三个接口。产品级分析由 LangGraph 驱动的 4 节点 DAG 生成，成分级分析由独立的离线 LangGraph workflow 预先生成写库，在线消费。
...
├── workflow_ingredient_analysis/      # [NEW] 离线：IngredientAnalysis 生成 workflow
│   ├── __init__.py
│   ├── types.py                       # [NEW] 输入/输出契约 TypedDict
│   ├── nodes.py                       # [NEW] 5 个 LangGraph 节点
│   └── graph.py                       # [NEW] LangGraph 编译图
├── db_repositories/
...
└── tests/
    ...
    │       ├── test_nodes.py
    │       └── test_graph.py
    ├── workflow_ingredient_analysis/   # [NEW]
    │   ├── __init__.py
    │   ├── test_nodes.py
    │   └── test_graph.py
    ├── db_repositories/
...
# ── 离线 IngredientAnalysis workflow 模型 ─────────────────────────────────
...
**目标：** 在 `analysis/types.py` 中定义管道各组件间流转的所有数据类型，作为整个分析模块的类型中枢。

**文件：**
- Create: `server/analysis/types.py`
...
**步骤：**
- [ ] 创建 `server/analysis/__init__.py`（空文件）
- [ ] 创建 `server/analysis/types.py`，定义上述所有类型
- [ ] 运行 `uv run python3 -c "from analysis.types import ProductAnalysisResult, AnalysisTask; print('OK')"`
...
**目标：** 实现 `analysis/redis_store.py`，封装对 Redis 中分析任务 JSON 的读写，提供创建、更新、查询任务状态的接口。

**文件：**
- Create: `server/analysis/redis_store.py`
- Create: `server/tests/analysis/test_redis_store.py`
...
- [ ] 创建 `server/analysis/redis_store.py`，实现上述 5 个函数
- [ ] 创建 `server/tests/analysis/__init__.py` 和 `server/tests/analysis/test_redis_store.py`
- [ ] 运行 `uv run pytest tests/analysis/test_redis_store.py -v`，确保全部通过
...
**目标：** 实现 `analysis/ocr_client.py`，向独立部署的 PaddleOCR-VL-1.5 服务发送图片，返回原始 OCR 文字。

**文件：**
- Create: `server/analysis/ocr_client.py`
- Create: `server/tests/analysis/test_ocr_client.py`
...
- [ ] 创建 `server/analysis/ocr_client.py`，实现 `run_ocr` 和 `OcrServiceError`
- [ ] 创建 `server/tests/analysis/test_ocr_client.py`
- [ ] 运行 `uv run pytest tests/analysis/test_ocr_client.py -v`
...
**目标：** 实现 `analysis/ingredient_parser.py`，用 LLM + Instructor 从 OCR 文字中提取成分名列表（兼做商品品名提取用于 food_id 匹配）。

**文件：**
- Create: `server/analysis/ingredient_parser.py`
- Create: `server/tests/analysis/test_ingredient_parser.py`
...
**步骤：**
- [ ] 创建 `server/analysis/ingredient_parser.py`
- [ ] 创建 `server/tests/analysis/test_ingredient_parser.py`
- [ ] 运行 `uv run pytest tests/analysis/test_ingredient_parser.py -v`
...
**目标：** 实现 `analysis/ingredient_matcher.py`，将成分名列表通过向量检索匹配到配料库（`ingredients` 表），返回 matched + unmatched 结果。

**文件：**
- Create: `server/analysis/ingredient_matcher.py`
- Create: `server/tests/analysis/test_ingredient_matcher.py`
...
**步骤：**
- [ ] 创建 `server/analysis/ingredient_matcher.py`
- [ ] 创建 `server/tests/analysis/test_ingredient_matcher.py`
- [ ] 运行 `uv run pytest tests/analysis/test_ingredient_matcher.py -v`
...
**目标：** 在 `analysis/product_agent/` 中定义 LangGraph State 和 4 个节点函数，每个节点通过 Instructor 调用 LLM 生成结构化输出。

**文件：**
- Create: `server/analysis/product_agent/__init__.py`
- Create: `server/analysis/product_agent/types.py`
- Create: `server/analysis/product_agent/nodes.py`
- Create: `server/tests/analysis/product_agent/test_nodes.py`

**`types.py` 定义：**

```python
class ProductAnalysisState(TypedDict):
    ingredients: list[IngredientInput]      # 输入（来自 analysis/types.py）
...
**步骤：**
- [ ] 创建 `server/analysis/product_agent/__init__.py`
- [ ] 创建 `server/analysis/product_agent/types.py`
- [ ] 创建 `server/analysis/product_agent/nodes.py`，实现 4 个节点函数
- [ ] 创建 `server/tests/analysis/product_agent/__init__.py` 和 `test_nodes.py`
- [ ] 运行 `uv run pytest tests/analysis/product_agent/test_nodes.py -v`
...
**目标：** 在 `analysis/product_agent/graph.py` 中将 4 个节点按 DAG 拓扑（A/B 并行 → C → D）编译为 LangGraph 图，并提供顶层调用入口。

**文件：**
- Create: `server/analysis/product_agent/graph.py`
- Create: `server/tests/analysis/product_agent/test_graph.py`
...
**步骤：**
- [ ] 创建 `server/analysis/product_agent/graph.py`
- [ ] 创建 `server/tests/analysis/product_agent/test_graph.py`
- [ ] 运行 `uv run pytest tests/analysis/product_agent/test_graph.py -v`
...
**目标：** 实现 `analysis/assembler.py`，将管道各阶段产物（DB 记录 + MatchResult + Agent 输出）组装成 `ProductAnalysisResult`，供 API 层直接返回。

**文件：**
- Create: `server/analysis/assembler.py`
- Create: `server/tests/analysis/test_assembler.py`
...
**步骤：**
- [ ] 创建 `server/analysis/assembler.py`
- [ ] 创建 `server/tests/analysis/test_assembler.py`
- [ ] 运行 `uv run pytest tests/analysis/test_assembler.py -v`
...
**文件：**
- Create: `server/analysis/food_resolver.py`
- Create: `server/tests/analysis/test_food_resolver.py`
...
**步骤：**
- [ ] 创建 `server/analysis/food_resolver.py`
- [ ] 创建 `server/tests/analysis/test_food_resolver.py`
- [ ] 运行 `uv run pytest tests/analysis/test_food_resolver.py -v`
...
**目标：** 实现 `analysis/pipeline.py`，将四组件串联为完整的异步任务函数，在 BackgroundTask 中执行，通过 Redis 实时更新状态。

**文件：**
- Create: `server/analysis/pipeline.py`
- Create: `server/tests/analysis/test_pipeline.py`
...
**步骤：**
- [ ] 创建 `server/analysis/pipeline.py`
- [ ] 创建 `server/tests/analysis/test_pipeline.py`
- [ ] 运行 `uv run pytest tests/analysis/test_pipeline.py -v`
...
## Task 17：IngredientAnalysis 离线 Workflow（Ingredient Analysis Workflow）

**目标：** 实现 `workflow_ingredient_analysis/`，一个独立的 LangGraph workflow，将单条 `IngredientDetail` 转换为 `IngredientAnalysis` 记录写库。此 workflow 由外部（CLI / admin API）触发，不参与在线请求链路。

**文件：**
- Create: `server/workflow_ingredient_analysis/__init__.py`
- Create: `server/workflow_ingredient_analysis/types.py`
- Create: `server/workflow_ingredient_analysis/nodes.py`
- Create: `server/workflow_ingredient_analysis/graph.py`
- Create: `server/tests/workflow_ingredient_analysis/__init__.py`
- Create: `server/tests/workflow_ingredient_analysis/test_nodes.py`
- Create: `server/tests/workflow_ingredient_analysis/test_graph.py`

**`types.py` 定义：**

```python
class IngredientAnalysisInput(TypedDict):
    ingredient_detail_id: int    # 业务入口键

class RunContext(TypedDict):
    run_id: str                  # 用于日志追踪

WorkflowStatus = Literal["queued", "running", "succeeded", "failed", "skipped"]

class IngredientAnalysisState(TypedDict):
    # 输入
    ingredient_detail_id: int
    run_id: str
    force_rerun: bool            # True 时跳过幂等检查
    # 中间产物
    detail_snapshot: dict | None        # load_detail_node 产出
    evidence_refs: list[dict] | None    # retrieve_evidence_node 产出
    level: IngredientRiskLevel | None   # analyze_node 产出
    confidence_score: float | None
    decision_trace: dict | None
    # 最终产物
    safety_info: str | None             # compose_output_node
    alternatives: list[dict] | None
    # 执行状态
    status: WorkflowStatus
    error_code: str | None
    analysis_version: str | None        # 由 version resolver 生成
    ai_model: str | None
```
...
```python
async def load_detail_node(state: IngredientAnalysisState, session: AsyncSession) -> IngredientAnalysisState
# 按 state.ingredient_detail_id 拉取 IngredientDetail 快照
# 未找到 → state.status="failed", state.error_code="detail_not_found"; 后续节点跳过
# 找到 → state.detail_snapshot = {id, name, ...}

async def check_idempotency_node(state: IngredientAnalysisState, session: AsyncSession) -> IngredientAnalysisState
# 幂等检查（在 load_detail 之后执行）
# 若 state.force_rerun=True → 跳过检查，直接进 retrieve
# 若存在 is_active=True 且 analysis_version=resolve_version() 的记录 → state.status="skipped"; 后续节点跳过
# 否则 → 继续

async def retrieve_evidence_node(state: IngredientAnalysisState, settings: Settings) -> IngredientAnalysisState
# 从知识库（ChromaDB + FTS）检索该成分相关证据
# 复用 kb/retriever/ 中已有检索逻辑
# 无证据时降级（不报错）：state.evidence_refs=[], 在 analyze_node 中允许产出 unknown
# 知识库不可用 → state.evidence_refs=[], 打 warning 日志

async def analyze_node(state: IngredientAnalysisState, settings: Settings) -> IngredientAnalysisState
# 输入：detail_snapshot + evidence_refs
# LLM + Instructor：推断 level（t0-t4 或 unknown）、confidence_score、decision_trace
# 使用 settings.INGREDIENT_ANALYSIS_MODEL

async def compose_output_node(state: IngredientAnalysisState, settings: Settings) -> IngredientAnalysisState
# 输入：detail_snapshot + evidence_refs + level + decision_trace
# LLM + Instructor：生成 safety_info（TEXT 摘要）和 alternatives（结构化候选列表）
# alternatives 候选必须引用配料库中已有的 ingredient_id（可通过向量检索获取候选）

async def persist_version_node(state: IngredientAnalysisState, session: AsyncSession, settings: Settings) -> IngredientAnalysisState
# 调用 db_repositories/ingredient_analysis.insert_new_version
# 成功 → state.status="succeeded"
# 失败 → state.status="failed", state.error_code="persist_failed"
```

**`graph.py` — 图编译：**

```python
def build_ingredient_analysis_graph(settings: Settings) -> CompiledGraph
# 节点顺序（线性，无并行）：
# START → load_detail → check_idempotency → retrieve_evidence → analyze → compose_output → persist_version → END
# 每个节点检查前序 status 若为 failed/skipped 则直接 passthrough（不执行本节点逻辑）
...
async def run_ingredient_analysis(
    ...
    session: AsyncSession = ...,
    settings: Settings = ...,
) -> IngredientAnalysisState
# 初始化 State，调用 graph.ainvoke，返回最终 state
# 调用方检查 state.status 确认 succeeded / failed / skipped
```

**版本决议辅助函数：**
```python
def resolve_analysis_version(settings: Settings) -> str
# 返回 settings.INGREDIENT_ANALYSIS_VERSION（MVP 固定为 "v1"）
# 未来可接入 model_id + prompt_hash 生成
```

**测试：**

`test_nodes.py`: Mock DB session、LLM 客户端，测试每个节点的正常路径和错误路径（detail_not_found、evidence_unavailable 降级、persist_failed）

`test_graph.py`: Mock 所有节点，测试：
- 正常 happy path → status="succeeded"
- load_detail 失败 → status="failed"
- force_rerun=False 且已有 active 记录 → status="skipped"
- force_rerun=True 且已有 active 记录 → 重跑，status="succeeded"

**关于 `ingredient_detail_id` vs `ingredient_id`：**
当前 `database/models.py` 中存在 `Ingredient` 表（对应 `ingredients`）。Workflow spec 引用的 `ingredient_detail` 可能等同于此表中的 `Ingredient` 记录（含 `description`、`safety_info`、`notes` 等字段）。实现时确认 `ingredient_detail_id == ingredient_id`（来自 `ingredients.id`），若不同则需先查明 `ingredient_details` 表的定义。

**可观测性（spec §9 最小集，在 `persist_version_node` 和 graph 入口埋点）：**
```python
# 使用 observability/metrics.py 已有的 prometheus client 模式
ingredient_analysis_run_total{status, version}      # succeeded / failed / skipped
ingredient_analysis_duration_ms{version}             # graph.ainvoke 总耗时
ingredient_analysis_unknown_rate{version}            # level="unknown" 的比率
ingredient_analysis_force_rerun_total{version}       # force_rerun=True 的调用次数
```
日志字段必须包含：`ingredient_detail_id`、`analysis_version`、`run_id`、`error_code`（失败时）。

**步骤：**
- [ ] 创建 `server/workflow_ingredient_analysis/` 目录及所有文件
- [ ] 创建测试目录及文件
- [ ] 运行 `uv run pytest tests/workflow_ingredient_analysis/ -v`
- [ ] commit: `feat(workflow): add IngredientAnalysis offline LangGraph workflow`
...
**文件：**
- Create: `server/tests/analysis/test_e2e_smoke.py`
...
**步骤：**
- [ ] 创建 `server/tests/analysis/test_e2e_smoke.py`
- [ ] 运行 `uv run pytest tests/analysis/test_e2e_smoke.py -v`
