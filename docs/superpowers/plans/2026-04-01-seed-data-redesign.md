...
**Goal:** 重写 `server/scripts/seed_data.py`，适配新数据库模型，移除 AnalysisDetail 生成，新增 IngredientAlias 和 IARC 相关表模拟数据。

**Architecture:** 单文件修改，所有数据生成逻辑在 `seed_data.py` 中。数据按依赖顺序：Ingredient → NutritionTable → Food → FoodIngredient → FoodNutritionEntry → IngredientAlias → IarcAgent → IarcCancerSite → IarcAgentLink。

**Tech Stack:** Python, Faker, SQLAlchemy AsyncSession, PostgreSQL
...
| 操作 | 说明 |
|------|------|
| 修改 | `server/scripts/seed_data.py` |

### 需更新的 import

从 `database.models` 新增导入：
- `IarcAgent`
- `IarcCancerSite`
- `IarcAgentLink`
- `IngredientAlias`
...
| 表 | 旧数量 | 新数量 |
|---|---|---|
| `NutritionTable` | 30 | 15 |
| `AnalysisDetail` | 生成 | **移除** |
| `IngredientAlias` | 无 | 每配料 0~3 条 |
| `IarcAgent` | 无 | 10 条 |
| `IarcCancerSite` | 无 | 5 条 |
| `IarcAgentLink` | 无 | 每 Agent 0~2 条 |

### Ingredient 新增字段（seed 时填充）
- `cas`: 格式 `XXX-XX-X`
- `applications`: 字符串
- `notes`: Text
- `safety_info`: Text
...
旧顺序含 `AnalysisDetail`，新顺序为：
1. `FoodNutritionEntry`
2. `FoodIngredient`
3. `Food`
4. `Ingredient`
5. `NutritionTable`
...
- [ ] **Task 1: 完全重写 `seed_data.py`**

完整重写文件，保持原有风格，新增所有数据生成函数和主逻辑。
...
预期输出包含所有表的插入完成信息，无报错。

- [ ] **Task 3: 提交代码**

```bash
git add server/scripts/seed_data.py && git commit -m "feat(db): rewrite seed_data.py for new models

- Remove AnalysisDetail generation
- Add IngredientAlias (0~3 per ingredient)
- Add IarcAgent/IarcCancerSite/IarcAgentLink (simulated data)
- Reduce NutritionTable to 15 records
