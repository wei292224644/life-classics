...
- `.claude/agents/working-harness-team-agents/facilitator.md` — 主任务层编排 agent
- `.claude/agents/working-harness-team-agents/decomposer.md` — 子任务拆分 agent
- `.claude/agents/working-harness-team-agents/context-manager.md` — 上下文同步 agent
- `.claude/agents/working-harness-team-agents/coder.md` — 代码实现 agent
- `.claude/agents/working-harness-team-agents/reviewer.md` — 代码审查 agent
- `.claude/agents/working-harness-team-agents/tester.md` — 测试 agent
- `.claude/agents/working-harness-team-agents/evaluator.md` — Sprint 合约 + 验收 agent
- `.claude/skills/working-harness-team/spec/SKILL.md` — 包装 brainstorming
- `.claude/skills/working-harness-team/plan/SKILL.md` — 包装 writing-plans
- `.claude/skills/working-harness-team/facilitator/SKILL.md` — 流程编排逻辑
- `.claude/skills/working-harness-team/code-review-rubric/SKILL.md` — 4 维评分
...
### Task 3: 创建 working-harness-team:spec skill

**Files:**
- Create: `.claude/skills/working-harness-team/spec/SKILL.md`

- [ ] **Step 1: 创建目录**

```bash
mkdir -p .claude/skills/working-harness-team/spec
```
...
---
name: spec
description: 在 brainstorming 基础上增加约束，产出格式规范的 spec.md。人工阶段使用，用于与 working-harness-team 执行阶段交接。
---

# working-harness-team: Spec

本 skill 包装 `superpowers:brainstorming`，在其基础上增加以下约束：

## 使用时机

用户需要为一个需求产出 spec.md 时使用，作为后续 working-harness-team 执行阶段的输入。
...
spec.md 完成并经用户确认后，使用 `working-harness-team:plan` 产出 plan.md，两者共同作为 facilitator 的输入。
```

- [ ] **Step 3: 验证文件存在**

```bash
cat .claude/skills/working-harness-team/spec/SKILL.md
```
...
```bash
git add .claude/skills/working-harness-team/spec/SKILL.md
git commit -m "feat: add working-harness-team:spec skill"
```

---

### Task 4: 创建 working-harness-team:plan skill

**Files:**
- Create: `.claude/skills/working-harness-team/plan/SKILL.md`

- [ ] **Step 1: 创建目录**

```bash
mkdir -p .claude/skills/working-harness-team/plan
```
...
```bash
cat .claude/skills/working-harness-team/plan/SKILL.md
```
...
```bash
git add .claude/skills/working-harness-team/plan/SKILL.md
git commit -m "feat: add working-harness-team:plan skill"
```

---

### Task 5: 创建 working-harness-team:facilitator skill

**Files:**
- Create: `.claude/skills/working-harness-team/facilitator/SKILL.md`

- [ ] **Step 1: 创建目录**

```bash
mkdir -p .claude/skills/working-harness-team/facilitator
```
...
---
name: facilitator
description: working-harness-team 执行阶段的主理编排逻辑。读取 spec.md + plan.md，协调 7 个 agent 按 Sprint 合约流程完成开发任务。
---

# working-harness-team: Facilitator

## 职责

编排 working-harness-team 执行阶段的完整流程。不写代码，不做技术判断，只做流程决策。
...
如不存在，告知用户先使用 `working-harness-team:spec` 和 `working-harness-team:plan` 产出这两个文件。

## 执行流程

### 阶段 1：初始化

1. 读取 `{RUN_DIR}/spec.md` 和 `{RUN_DIR}/plan.md`
2. 用自己的话复述理解（一段话），询问用户确认
3. 用户确认后，调用 decomposer agent 产出 `{RUN_DIR}/subtasks.md`
4. 调用 context-manager agent 初始化 `{RUN_DIR}/context.md`
...
2. 调用 evaluator 起草 `{RUN_DIR}/tasks/{task-id}/sprint-contract.md`
3. 调用 coder 确认合约并实现，产出代码 + `{RUN_DIR}/tasks/{task-id}/handoff.md`
4. 调用 reviewer 审查，产出 `{RUN_DIR}/tasks/{task-id}/review.md`
5. 如有 blocking 问题，coder 修改（最多 3 轮）
6. 调用 tester 写测试并执行，产出 `{RUN_DIR}/tasks/{task-id}/test-result.md`
7. 调用 evaluator 验收，产出 `{RUN_DIR}/tasks/{task-id}/verdict.md`
...
1. 所有子任务完成后，写 `{RUN_DIR}/summary.md`
...
```bash
cat .claude/skills/working-harness-team/facilitator/SKILL.md
```

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/working-harness-team/facilitator/SKILL.md
git commit -m "feat: add working-harness-team:facilitator skill"
```

---

### Task 6: 创建 working-harness-team:code-review-rubric skill

**Files:**
- Create: `.claude/skills/working-harness-team/code-review-rubric/SKILL.md`

- [ ] **Step 1: 创建目录**

```bash
mkdir -p .claude/skills/working-harness-team/code-review-rubric
```
...
# working-harness-team: Code Review Rubric
...
```bash
cat .claude/skills/working-harness-team/code-review-rubric/SKILL.md
```

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/working-harness-team/code-review-rubric/SKILL.md
git commit -m "feat: add working-harness-team:code-review-rubric skill"
```
...
**Files:**
- Create: `.claude/agents/working-harness-team-agents/facilitator.md`
- Create: `.claude/agents/working-harness-team-agents/decomposer.md`
- Create: `.claude/agents/working-harness-team-agents/context-manager.md`
- Create: `.claude/agents/working-harness-team-agents/coder.md`
- Create: `.claude/agents/working-harness-team-agents/reviewer.md`
- Create: `.claude/agents/working-harness-team-agents/tester.md`
- Create: `.claude/agents/working-harness-team-agents/evaluator.md`

- [ ] **Step 1: 创建目录**

```bash
mkdir -p .claude/agents/working-harness-team-agents
```
...
读取 `{RUN_DIR}/plan.md`，将其拆分为结构化的子任务列表。
...
description: 维护 {RUN_DIR}/context.md 共享上下文快照。每个子任务开始前和结束后各同步一次。
tools: Read, Write
---
...
你是 working-harness-team 的 coder（实现）agent。
...
你是 working-harness-team 的 tester（测试）agent。
...
你是 working-harness-team 的 evaluator（评估）agent。

## 双重职责

### 职责 1：起草 Sprint 合约（子任务开始前）

读取 `{RUN_DIR}/spec.md` + `{RUN_DIR}/subtasks.md`，为当前子任务起草 sprint-contract.md。
...
```bash
ls .claude/agents/working-harness-team-agents/
```
...
```bash
git add .claude/agents/working-harness-team-agents/
git commit -m "feat: add 7 agent configs for working-harness-team"
```
...
注意：`.agent-workspace/runs/` 下的具体 run 目录由 facilitator 在运行时创建，无需预先创建。
...
注意：只忽略 runs 目录，保留 `.agent-workspace/` 目录结构本身（`.gitkeep`）。
...
在 `.agent-workspace/runs/` 下创建测试 run 目录并写入文件：

```bash
mkdir -p .agent-workspace/runs/test-kb-stats
```

将以下内容写入 `.agent-workspace/runs/test-kb-stats/spec.md`：
...
将以下内容写入 `.agent-workspace/runs/test-kb-stats/plan.md`：
...
```
RUN_DIR=.agent-workspace/runs/test-kb-stats
请读取 {RUN_DIR}/spec.md 和 {RUN_DIR}/plan.md，开始执行。
```
...
```bash
# 应按顺序出现
ls .agent-workspace/runs/test-kb-stats/subtasks.md
ls .agent-workspace/runs/test-kb-stats/context.md
ls .agent-workspace/runs/test-kb-stats/tasks/
```
...
```bash
# sprint-contract 存在
ls .agent-workspace/runs/test-kb-stats/tasks/*/sprint-contract.md

# handoff 存在
ls .agent-workspace/runs/test-kb-stats/tasks/*/handoff.md

# verdict 存在且为 pass
cat .agent-workspace/runs/test-kb-stats/tasks/*/verdict.md | grep "pass"
```
...
```bash
rm -rf .agent-workspace/runs/test-kb-stats/
```
...
```bash
git add -A
git commit -m "feat: working-harness-team workflow complete and verified"
```
...
- [ ] `.claude/skills/working-harness-team/` 下 4 个 skill 文件存在
- [ ] `.claude/agents/working-harness-team-agents/` 下 7 个 agent 配置文件存在
