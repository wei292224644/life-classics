...
**实现逻辑**：
- 遍历该文档所有 chunks，更新 ChromaDB 中每个 chunk metadata 里的对应字段
- 只更新传入的字段，未传字段保持原值
- 若该文档无任何 chunks，返回 404
...
- 挂载 `DocEditDrawer`，`onSaved` 时调用 `refreshDocuments()`
...
复用 `Sheet` 组件，与 `ChunkEditDrawer` 保持一致的交互模式。
...
- `title`：`Input`
- `standard_no`：`Input`
- `doc_type`：`Input`（不用 Select，类型值不固定）

**保存流程**：
1. 调用 `api.documents.update(doc_id, payload)`
2. 成功：toast「已保存」→ 调用 `onSaved(updated)` 刷新列表 → 关闭抽屉
3. 失败：toast「保存失败」+ 错误信息

### 4. `ChunkEditDrawer.tsx`

在现有字段下方、`raw_content` 之后，新增只读分组「文档元数据（只读）」，展示：
- `doc_id`
- `title`（文档标题）
- `standard_no`
- `doc_type`

样式：与现有 `raw_content` 只读块一致，使用 `bg-muted` 灰底 + `text-muted-foreground` 文字色。
...
| 只有 1 页 | 隐藏分页控件 |
| 文档无 chunks | 后端返回 404，前端 toast 提示 |
