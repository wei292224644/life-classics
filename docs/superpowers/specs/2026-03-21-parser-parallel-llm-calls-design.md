...
- **行为变更**：原本某个 chunk LLM 失败会导致整个 node 失败；改动后为部分失败——失败的 chunk 记录到 `errors`，其余正常继续

---

## 方案 A（推荐）：全并发（asyncio.gather）

### 前置条件：node 函数改为 async

LangGraph 的 `ainvoke` 调用节点时，节点必须是 `async def` 函数。当前 `classify_node` 和 `transform_node` 都是 `def`，需要改为 `async def`。
...
- `return_exceptions=True` 捕获的异常 append 到 `state["errors"]`
- errors 字段格式为 `List[str]`：`"{node_name}[{chunk_index}]: {exception}"`
- `state["errors"]` 贯穿流水线，最终 `ParserResult.errors` 包含所有错误字符串
- `stats["error_count"]` 在 `ParserResult` 中反映总错误数量
...
2. **集成测试**：使用 mock patch LLM，验证并发确实发生且顺序无关
