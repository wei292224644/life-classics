...
| 规模 | 特征 | 跳过步骤 |
|------|------|---------|
| 小（bug fix）| 单文件改动，无新接口 | 无需 sprint-contract 协商，直接 coder → evaluator |
| 中（新功能）| 单 workspace，有新接口 | 完整流程 |
| 大（跨 workspace）| 多 workspace | 拆分为多个中任务串行执行 |
...
2. 停止实现，等待 facilitator 仲裁
3. facilitator 仲裁：接受替代方案 → 更新合约，通知 coder 继续；拒绝 → 通知 coder 必须实现
