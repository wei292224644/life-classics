...
async def test_full_parser_workflow_with_real_llm_logs_all_steps():
    ...
    # 通过 LangGraph streaming 逐节点记录日志，同时积累全流程状态
    ...
    try:
        async for chunk in parser_graph.astream(
            ...
            for node_name, node_output in chunk.items():
                ...
                if node_name == "parse_node":
                    ...
                    logger.info(
                        "escalate_node applied: reclassified all unknown segments"
                    )
                elif node_name == "transform_node":
                    logger.info(
        ...
        # 统一记录工作流失败时的节点与原因，便于从测试日志快速定位问题。
        logger.exception("parser_graph astream failed with error: %s", exc)
        raise
    ...
    #     if not str(err).startswith(("INFO:", "WARN:"))
    # ]
