...
from app.core.kb.writer.fts_writer import delete_by_standard_no, init_db, write
...
def _make_meta(standard_no: str = "GB/T-001") -> dict:
    return {"standard_no": standard_no}
...
def test_write_stores_chunk_in_base_table(tmp_path):
    ...
    write([chunk], _make_meta("GB 5009.1"), db_path=db)

    with sqlite3.connect(db) as conn:
        row = conn.execute(
            "SELECT chunk_id, standard_no, semantic_type, section_path FROM chunks WHERE chunk_id = ?",
            ("abc123",),
        ).fetchone()

    assert row is not None
    assert row[0] == "abc123"
    assert row[1] == "GB 5009.1"
    assert row[2] == "scope"
    assert row[3] == "1|1.1"
...
def test_write_fts_match_returns_chunk(tmp_path):
    ...
    write([chunk], _make_meta("GB 2760"), db_path=db)
...
def test_write_multiple_chunks(tmp_path):
    ...
    write(chunks, _make_meta("GB/T-010"), db_path=db)

    with sqlite3.connect(db) as conn:
        count = conn.execute(
            "SELECT COUNT(*) FROM chunks WHERE standard_no = ?", ("GB/T-010",)
        ).fetchone()[0]
...
def test_delete_removes_records(tmp_path):
    ...
    write(chunks, _make_meta("GB/T-DEL"), db_path=db)

    errors: list = []
    result = delete_by_standard_no("GB/T-DEL", errors, db_path=db)
    ...
    with sqlite3.connect(db) as conn:
        count = conn.execute(
            "SELECT COUNT(*) FROM chunks WHERE standard_no = ?", ("GB/T-DEL",)
        ).fetchone()[0]
        fts_count = conn.execute(
...
def test_delete_nonexistent_standard_no_returns_true(tmp_path):
    """删除不存在的 standard_no 视为成功，返回 True。"""
    db = str(tmp_path / "test.db")
    init_db(db_path=db)

    errors: list = []
    result = delete_by_standard_no("NOT_EXIST", errors, db_path=db)

    assert result is True
    assert errors == []


def test_delete_only_removes_target_standard(tmp_path):
    """删除某个 standard_no 不影响其他 standard_no 的记录。"""
    db = str(tmp_path / "test.db")
    init_db(db_path=db)

    write([_make_chunk(chunk_id="keep1", content="保留内容")], _make_meta("GB/T-KEEP"), db_path=db)
    write([_make_chunk(chunk_id="del1", content="删除内容")], _make_meta("GB/T-DEL2"), db_path=db)

    errors: list = []
    delete_by_standard_no("GB/T-DEL2", errors, db_path=db)

    with sqlite3.connect(db) as conn:
        keep_count = conn.execute(
            "SELECT COUNT(*) FROM chunks WHERE standard_no = ?", ("GB/T-KEEP",)
        ).fetchone()[0]
        del_count = conn.execute(
            "SELECT COUNT(*) FROM chunks WHERE standard_no = ?", ("GB/T-DEL2",)
        ).fetchone()[0]
...
def test_delete_returns_false_on_error(tmp_path):
    ...
    # 使用一个不存在且无法创建的路径来触发错误
    errors: list = []
    # 传入无效路径触发异常（目录不存在且 init_db 未调用）
    result = delete_by_standard_no("GB/T-ERR", errors, db_path="/nonexistent_dir/test.db")
