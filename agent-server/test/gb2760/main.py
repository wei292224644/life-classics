...
你的任务是：
将来自 PDF 版式解析结果（基于坐标的文本数组）的内容，
转换为结构化的 Markdown 格式，保留原文的层次结构和表格信息。

输入数据格式为列表数组，每个元素表示一个文本片段：
(x0, y0, x1, y1, text, line_id, block_id)
...
4. 不要修改原文内容
5. 输出标准的 Markdown 格式""",
    "markdown": """你是一个文档格式化专家。

你的任务是：
将来自 PDF 版式解析结果（基于坐标的文本数组）的内容，
转换为清晰易读的 Markdown 格式。
...
def parse_page_range_config(
    ...
    Args:
        page_ranges: 页数范围列表，格式为：
            [{"pages": [1, 2], "prompt": "提示词"}, {"pages": [3], "prompt": "提示词"}]
...
if __name__ == "__main__":
    ...
    page_ranges = [
        {"pages": [156], "prompt": PROMPT_MODES["dsl"]},
    ]
