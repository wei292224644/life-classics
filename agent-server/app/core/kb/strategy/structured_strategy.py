...
class StructuredMarkdownParser:
    ...
    def _create_chunk(
        ...
        # 将字符串 content_type 转换为 ContentType 枚举
        try:
            content_type_enum = ContentType(content_type)
        except ValueError:
            # 如果无法转换，使用 NOTE 作为默认值
            content_type_enum = ContentType.NOTE
...
def split_structured(documents: List[DocumentChunk], **kwargs) -> List[DocumentChunk]:
    ...
    Returns:
        切分后的 DocumentChunk 对象列表
    """
    parser = StructuredMarkdownParser()
    all_chunks = []

    for document in documents:
        # 从 DocumentChunk 中提取 Markdown 内容
        markdown_content = str(document.content) if document.content else ""

        if not markdown_content.strip():
            continue

        # 解析 Markdown 内容
        chunks = parser.parse_markdown(
