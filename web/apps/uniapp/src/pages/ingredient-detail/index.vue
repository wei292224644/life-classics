<template>
  <view class="h-screen bg-[var(--bg-base)] flex flex-col relative overflow-hidden">
    <!-- ── 自定义 Header ──────────────────────────── -->
    <view class="ing-header sticky top-0 z-100 flex-shrink-0" :class="riskClass">
      <!-- 状态栏占位（动态高度） -->
      <view :style="{ height: themeStore.statusBarHeight + 'px' }" />
      <view class="header-content flex items-center gap-4 px-6 pb-6">
        <button class="header-btn w-[80rpx] h-[80rpx] rounded-3xl flex items-center justify-center flex-shrink-0" @click="goBack">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
            <path d="M15 18l-6-6 6-6"/>
          </svg>
        </button>
        <view class="header-text flex-1 flex flex-col gap-1">
          <text class="header-title">配料详情</text>
          <text class="header-subtitle">{{ headerSubtitle }}</text>
        </view>
        <button class="header-btn w-[80rpx] h-[80rpx] rounded-3xl flex items-center justify-center flex-shrink-0" @click="shareToFriend">
    ...
    <!-- ── 加载态 ──────────────────────────────── -->
    <view v-if="isLoading" class="loading-state flex-1 flex items-center justify-center">
      <text class="loading-text">加载中...</text>
    </view>

    <!-- ── 无数据错误态 ──────────────────────────── -->
    <view v-else-if="!ingredient" class="error-state flex-1 flex flex-col items-center justify-center gap-8 px-12">
      <text class="error-text">数据加载失败，请返回重试</text>
      <button class="retry-btn" @click="goBack">返回</button>
    </view>

    <!-- ── 内容区 ────────────────────────────────── -->
    <scroll-view v-else scroll-y class="scroll-area flex-1 overflow-hidden w-full">
    <view class="scroll-content px-6 pt-6">

      <!-- Hero 风险卡 -->
      <view class="section-card hero-card bg-[var(--bg-card)] rounded-xl overflow-hidden border border-[var(--border-color)] shadow-[var(--shadow-sm)] mb-6">
        <view class="hero-top" :style="heroTopStyle">
          <view class="hero-name-row flex items-start justify-between gap-4 mb-4">
            <view class="hero-name-wrap flex-1 flex flex-col gap-1">
            ...
            <view class="risk-badge flex items-center gap-2 px-5 py-2 rounded flex-shrink-0" :class="riskClass">
        ...
        <!-- Chips -->
        <view class="chips-row flex flex-wrap gap-3 px-6 py-5">
      ...
      <!-- 描述 -->
      <view v-if="summary" class="section-card bg-[var(--bg-card)] rounded-xl px-8 pt-8 pb-6 mb-6 border border-[var(--border-color)] shadow-[var(--shadow-sm)]">
        <view class="section-header flex items-center gap-4 mb-5 pb-4 border-b border-[var(--border-color)]">
          ...
          <text class="section-title text-[var(--text-primary)] font-semibold text-lg flex-1">描述</text>
      ...
      <!-- AI 风险分析 -->
      <view v-if="riskFactors.length > 0" class="section-card bg-[var(--bg-card)] rounded-xl px-8 pt-8 pb-6 mb-6 border border-[var(--border-color)] shadow-[var(--shadow-sm)]">
        <view class="section-header flex items-center gap-4 mb-5 pb-4 border-b border-[var(--border-color)]">
          ...
          <text class="section-title text-[var(--text-primary)] font-semibold text-lg flex-1">AI 风险分析</text>
          <text class="ai-label">AI</text>
        </view>
        <view class="list-items flex flex-col gap-4">
          <view v-for="(item, i) in riskFactors" :key="i" class="list-item flex items-start gap-4">
      ...
      <!-- 风险管理信息 -->
      <view v-if="hasRiskMgmt" class="section-card bg-[var(--bg-card)] rounded-xl px-8 pt-8 pb-6 mb-6 border border-[var(--border-color)] shadow-[var(--shadow-sm)]">
        <view class="section-header flex items-center gap-4 mb-5 pb-4 border-b border-[var(--border-color)]">
          ...
          <text class="section-title text-[var(--text-primary)] font-semibold text-lg flex-1">风险管理信息</text>
        </view>
        <view class="kv-table flex flex-col gap-4">
          <view v-if="ingredient.who_level" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">WHO 致癌等级</text>
          ...
          <view v-if="maternalLevel" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">母婴等级</text>
            <text class="kv-value kv-value-red">{{ maternalLevel }}</text>
          </view>
          <view v-if="usageLimit" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">使用限量</text>
            <text class="kv-value">{{ usageLimit }}</text>
          </view>
          <view v-if="applicableRegion" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">适用区域</text>
            <text class="kv-value">{{ applicableRegion }}</text>
          </view>
          <view v-if="ingredient.allergen_info" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">过敏信息</text>
            <text class="kv-value">{{ ingredient.allergen_info }}</text>
          </view>
          <view v-if="ingredient.standard_code" class="kv-row flex items-start gap-4">
            <text class="kv-key flex-shrink-0">执行标准</text>
      ...
      <!-- AI 使用建议 -->
      <view v-if="suggestions.length > 0" class="section-card bg-[var(--bg-card)] rounded-xl px-8 pt-8 pb-6 mb-6 border border-[var(--border-color)] shadow-[var(--shadow-sm)]">
        <view class="section-header flex items-center gap-4 mb-5 pb-4 border-b border-[var(--border-color)]">
          ...
          <text class="section-title text-[var(--text-primary)] font-semibold text-lg flex-1">AI 使用建议</text>
          <text class="ai-label">AI</text>
        </view>
        <view class="list-items flex flex-col gap-4">
          <view v-for="(s, i) in suggestions" :key="i" class="list-item flex items-start gap-4">
            <view
      ...
      <!-- 含此配料的产品 -->
      <view v-if="relatedProducts.length > 0" class="section-card bg-[var(--bg-card)] rounded-xl px-8 pt-8 pb-6 mb-6 border border-[var(--border-color)] shadow-[var(--shadow-sm)]">
        <view class="section-header flex items-center gap-4 mb-5 pb-4 border-b border-[var(--border-color)]">
          ...
          <text class="section-title text-[var(--text-primary)] font-semibold text-lg flex-1">含此配料的产品</text>
        </view>
        <scroll-view scroll-x enable-flex class="related-scroll overflow-x-auto">
          <view class="related-inner flex flex-row gap-5 w-max pb-2">
            <view
              v-for="p in relatedProducts"
              :key="p.id"
              class="related-card flex-0-auto w-[172rpx] flex flex-col items-center gap-3"
              @click="goToProduct(p.barcode)"
            >
              <view class="related-img-wrap w-[172rpx] h-[172rpx] rounded-lg overflow-hidden bg-[var(--bg-base)] border border-[var(--border-color)]">
                <image
      ...
      <!-- 底部安全距离 -->
      <view class="bottom-spacer h-[180rpx]" />
      </view>
    </scroll-view>
...
// Risk level color classes — scoped to this component (CSS custom property definitions)
